  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-2' : 'bg-transparent py-4'}`}>
      <div className="container mx-auto px-4 md:px-8 flex items-center justify-between">
        ...
        >
          <div className={`relative w-10 h-10 rounded-full overflow-hidden border-2 transition-all ${scrolled ? 'border-emerald-600' : 'border-white'}`}>
            <img 
              src={LOGO_URL} 
              alt="Bu Aminah Logo" 
          ...
          <span className={`font-bold text-lg transition-colors ${scrolled ? 'text-amber-900' : 'text-white'}`}>
            Bu Aminah
          </span>
        </a>
        ...
        <div className="hidden lg:flex items-center gap-8">
          {[
            ...
            <a
              ...
              className={`font-medium transition-colors hover:text-emerald-500 ${scrolled ? 'text-stone-700' : 'text-white'}`}
            >
              {link.name}
            </a>
          ))}
          <a
            ...
            className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-full font-semibold transition-all shadow-lg hover:shadow-emerald-200"
          >
            Pesan Sekarang
          </a>
        </div>
