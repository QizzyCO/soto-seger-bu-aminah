...
import { LOGO_URL, WHATSAPP_RESERVATION_URL } from '../constants.ts';

interface NavbarProps {
  onHalalClick?: () => void;
}

const Navbar: React.FC<NavbarProps> = ({ onHalalClick }) => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-2' : 'bg-transparent py-4 md:py-5'}`}>
      <div className="container mx-auto px-4 md:px-8 flex items-center justify-between h-16">
        ...
        {/* Hamburger Button */}
        <button 
          className={`lg:hidden p-2 transition-colors focus:outline-none ${scrolled ? 'text-emerald-600' : 'text-white'}`}
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden absolute top-full left-0 w-full bg-white shadow-2xl transition-all duration-300 overflow-y-auto ${isOpen ? 'max-h-[85vh] opacity-100 border-t border-gray-100' : 'max-h-0 opacity-0 pointer-events-none'}`}>
        <div className="flex flex-col p-6 space-y-2">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-stone-700 font-bold hover:text-emerald-600 transition-colors py-3 border-b border-stone-50 text-center text-lg active:bg-emerald-50"
              onClick={(e) => handleLinkClick(e, link.href)}
            >
              {link.name}
            </a>
          ...
          <div className="pt-4 pb-2">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="block w-full bg-emerald-600 text-white text-center py-4 rounded-2xl font-bold shadow-lg hover:bg-emerald-700 transition-colors"
            >
              Pesan Sekarang
            </a>
          </div>
