...
import { MENU_ITEMS, MENU_PDF_URL } from '../constants.ts';
            ...
            <div key={cat} className="mb-20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {filteredItems.map((item) => (
                  ...
                  >
                    <div className={`relative ${cat === 'Minuman' ? 'h-64' : 'h-48'} overflow-hidden bg-stone-100`}>
                      <img 
                        src={item.image} 
                        alt={item.name} 
                        className="w-full h-full object-cover transition-all duration-500 group-hover:scale-110 opacity-100 brightness-100 contrast-100 group-hover:brightness-105 group-hover:contrast-105"
                        loading="lazy"
                      />
                      {item.popular && (
                        <div className="absolute top-3 right-3 bg-emerald-600 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-lg z-10">
                          POPULER
                        </div>
                      )}
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      ...
                      <div className="flex justify-end items-center mt-auto">
                        <div className="w-8 h-8 rounded-full bg-stone-50 flex items-center justify-center text-stone-400 group-hover:bg-emerald-600 group-hover:text-white transition-all shadow-sm">
